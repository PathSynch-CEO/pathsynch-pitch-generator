...
- Select repeated exact-SHA evidence by GitHub Actions check-suite creation time, bind required jobs and the skipped deploy guard to one workflow run, and reject pending, startup-failed, stale, cross-SHA, foreign-provider, incomplete, or indeterminately ordered evidence.
...
- Aligned the Scheduler availability adapter with the current documented Nylas v3 REST response,
  whose envelope contains Unix-second slots in `data.time_slots`. Existing slot duration, bounds,
  duplication, and malformed-response
...
- Added a sanitized real-shape regression fixture and strict coverage that rejects an undocumented
  direct `data` array. No booking, Nylas configuration change, deployment, or public API change is
  included.
...
- Added optimistic session updates, monotonically versioned availability receipts, hashed
  idempotency document keys, session-level single-winner operation reservations, durable
  slot/attendee verification snapshots, and fail-closed provider-pending and outcome-unknown states
  for the later Nylas adapter.
