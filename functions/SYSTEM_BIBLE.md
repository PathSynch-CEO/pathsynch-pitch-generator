...
For stacked or non-`main`-base candidates, the same `CI` workflow may be run manually after its `workflow_dispatch` trigger has landed on the default branch. The operator must dispatch against the candidate branch or immutable tag with a required lowercase 40-character `expected_sha`. Both required jobs compare that value with GitHub's dispatched `github.sha` and the checked-out `git rev-parse HEAD`, failing before dependency installation when any value differs. Dispatching the workflow on `main` while merely checking out another ref is not valid evidence because GitHub would attach the checks to the wrong commit. The authoritative run and both required checks must report the candidate SHA.

The manual path executes only the existing `Test & Audit` and `Emulator Tests (rules)` validation jobs. It does not grant deployment authority: `Deploy to Firebase` remains unconditionally disabled, permissions remain unchanged, and `pull_request_target` is forbidden.

Repeated runs for one reviewed SHA are valid. The critical-review evidence consumer fetches all GitHub check runs and check suites for that exact SHA, ignores checks from other SHAs and non-GitHub-Actions providers, and groups matching jobs by GitHub's check-suite identity. It selects the single suite with the newest server-generated `created_at`; equal creation times or incomplete identity metadata fail closed. Within one suite, a repeated same-name attempt is ordered by server-generated `started_at`, with ties or missing ordering metadata treated as ambiguous. Numeric IDs identify suites, runs, and jobs but are not assumed to encode chronology.

The selected suite must bind the two required names to one repository Actions run. A completed result is authoritative only when the same run also reports `Deploy to Firebase` as skipped. A newer failure, startup failure without job checks, pending run, or incomplete check set blocks GREEN and can never fall back to an older success. A completed successful suite without relevant check identities may be an unrelated workflow and does not supersede CI. The repository contract also forbids any other checked-in workflow from defining the required check names or the deploy-guard name, preventing unrelated suites from being combined into valid evidence.
